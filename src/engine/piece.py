...
PIECE_MOVE_PATTERNS = {
    ...
    PieceType.DAI: {  # 大 - 龍王の動き（飛車+王の斜め）
        'base': {
            # 縦横は無制限、斜めは1マス
            'moves': [[-1, 0], [1, 0], [0, -1], [0, 1],  # 縦横（飛車）
                     [-1, -1], [-1, 1], [1, -1], [1, 1]],  # 斜め1マス（王）
            'maxSteps': 9,  # 縦横は盤面端まで
            'canJump': False
        },
        'evolved': {
            # 2段目: 範囲拡大
            'moves': [[-1, 0], [1, 0], [0, -1], [0, 1],  # 縦横
                     [-1, -1], [-1, 1], [1, -1], [1, 1],  # 斜め1マス
                     [-2, -2], [-2, 2], [2, -2], [2, 2]],  # 斜め2マス
            'maxSteps': 9,
            'canJump': False
        },
        'mastered': {
            ...
            'moves': [[-1, 0], [1, 0], [0, -1], [0, 1],  # 縦横
                     [-1, -1], [-1, 1], [1, -1], [1, 1],  # 斜め1マス
                     [-2, -2], [-2, 2], [2, -2], [2, 2],  # 斜め2マス
                     [-3, -3], [-3, 3], [3, -3], [3, 3]],  # 斜め3マス
            'maxSteps': 9,
            'canJump': False
        }
    },
    PieceType.CHUU: {  # 中 - 龍馬の動き（角行+王の縦横）
        'base': {
            # 斜めは無制限、縦横は1マス
            'moves': [[-1, -1], [-1, 1], [1, -1], [1, 1],  # 斜め（角行）
                     [-1, 0], [1, 0], [0, -1], [0, 1]],  # 縦横1マス（王）
            'maxSteps': 9,  # 斜めは盤面端まで
            'canJump': False
        },
        'evolved': {
            # 2段目: 範囲拡大
            'moves': [[-1, -1], [-1, 1], [1, -1], [1, 1],  # 斜め
                     [-1, 0], [1, 0], [0, -1], [0, 1],  # 縦横1マス
                     [-2, 0], [2, 0], [0, -2], [0, 2]],  # 縦横2マス
            'maxSteps': 9,
            'canJump': False
        },
        'mastered': {
